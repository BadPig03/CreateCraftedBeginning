...
public class CCBAdvancements implements DataProvider {
    ...
    public static final CCBAdvancement TAKE_A_DEEP_BREATHE = create("take_a_deep_breathe", b -> b.icon(CCBBlocks.AIRTIGHT_PUMP_BLOCK).title("Take a Deep Breathe").description("Place and power an Airtight Pump").after(PLYWOOD));
    public static final CCBAdvancement WATER_HAMMER_EFFECT = create("water_hammer_effect", b -> b.icon(CCBBlocks.AIRTIGHT_ENCASED_PIPE_BLOCK).title("Water Hammer Effect").description("Obtain an Airtight Encased Pipe").whenIconCollected().after(TAKE_A_DEEP_BREATHE));
    ...
    public static final CCBAdvancement RISING_FORCE = create("rising_force", b -> b.icon(CCBBlocks.AIRTIGHT_ENGINE_BLOCK).title("Rising Force").description("Use an Airtight Engine to generate torque").after(UNDER_IMMENSE_PRESSURE));
    ...
    public static final CCBAdvancement STRIPPED = create("stripped", b -> b.icon(CCBItems.BREEZE_CORE).title("Stripped").description("Assemble a Breeze Core").whenIconCollected().after(UNIVERSAL_ANTIDOTE));
    public static final CCBAdvancement WIND_CHARGER = create("wind_charger", b -> b.icon(CCBItems.AIRTIGHT_CANNON).title("Wind Charger").description("Defeat any monster with an Airtight Cannon").after(STRIPPED));
    public static final CCBAdvancement MINI_TBM = create("mini_tbm", b -> b.icon(CCBItems.AIRTIGHT_HANDHELD_DRILL).title("Mini-TBM").description("Mine 64 blocks at the same time using an Airtight Handheld Drill").after(WIND_CHARGER));
