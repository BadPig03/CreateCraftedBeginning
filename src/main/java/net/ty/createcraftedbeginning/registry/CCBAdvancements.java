...
public class CCBAdvancements implements DataProvider {
    ...
    public static final CCBAdvancement HANDS_GET_DAZE = create("hands_get_daze", b -> b.icon(CCBItems.AIRTIGHT_EXTEND_ARM).title("Hands Get Da☆Ze").description("Assembly an Airtight Extend Arm").whenIconCollected().after(THE_SKIES_ARE_CLEARING_UP_NOW));
    public static final CCBAdvancement MINI_TBM = create("mini_tbm", b -> b.icon(CCBItems.AIRTIGHT_HANDHELD_DRILL).title("Mini-TBM").description("Mine 64 blocks at the same time using an Airtight Handheld Drill").after(HANDS_GET_DAZE));
