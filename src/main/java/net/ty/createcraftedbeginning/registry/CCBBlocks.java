...
public class CCBBlocks {
    ...
    public static final BlockEntry<CinderIncinerationBlowerBlock> CINDER_INCINERATION_BLOWER_BLOCK = CCB_REGISTRATE.block("cinder_incineration_blower", CinderIncinerationBlowerBlock::new).initialProperties(SharedProperties::softMetal).transform(CCBStress.setImpact(4f)).transform(pickaxeOnly()).transform(CCBBuilderTransformer.cinder_incineration_blower()).properties(p -> p.mapColor(MapColor.COLOR_YELLOW).noOcclusion()).register();
