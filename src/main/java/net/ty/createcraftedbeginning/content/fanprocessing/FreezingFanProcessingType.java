...
public class FreezingFanProcessingType implements FanProcessingType {
    ...
    public boolean canProcess(ItemStack stack, @NotNull Level level) {
        return level.getRecipeManager().getRecipeFor(CCBRecipeTypes.FREEZING.getType(), new SingleRecipeInput(stack), level).isPresent();
    }

    @Override
    public @Nullable List<ItemStack> process(ItemStack stack, @NotNull Level level) {
        return level.getRecipeManager().getRecipeFor(CCBRecipeTypes.FREEZING.getType(), new SingleRecipeInput(stack), level).map(recipe -> RecipeApplier.applyRecipeOn(level, stack, recipe.value(), true)).orElse(null);
    }
