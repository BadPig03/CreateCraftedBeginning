...
public final class AirtightPumpPressureNetwork {
    ...
    public static PressureDistributionResult distributePressureTo(AirtightPumpBlockEntity pump, Direction direction) {
    ...
    private record PressureTopologyEdge(Direction face, AdjacentTarget adjacentTarget, EndpointKind endpointKind, @Nullable PressureTopologyNode target) {
        private boolean isEndpoint(Level level, boolean isPulling) {
            return endpointKind == EndpointKind.STATIC || endpointKind == EndpointKind.ALIGNED_PUMP && level.getBlockEntity(adjacentTarget.pos()) instanceof AirtightPumpBlockEntity pump && pump.isPumpRunning() && AirtightPumpPressureController.isPullingOnSide(pump.isFront(adjacentTarget.connectedFace())) != isPulling;
        }
    }
