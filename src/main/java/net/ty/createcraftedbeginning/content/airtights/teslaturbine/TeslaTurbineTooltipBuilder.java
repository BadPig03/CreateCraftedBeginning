...
public class TeslaTurbineTooltipBuilder {
    ...
    private void addDetailedInfo(@NotNull List<Component> tooltip) {
        ...
        tooltip.add(CommonComponents.EMPTY);
        double totalStress = BASE_ROTATION_SPEED * Mth.abs(levelCalculator.getSpeed()) * levelCalculator.getCurrentLevel() * BlockStressValues.getCapacity(CCBBlocks.TESLA_TURBINE_BLOCK.get()) / AllConfigs.server().kinetics.maxRotationSpeed.get();
        CCBLang.translate("gui.goggles.capacity_provided").style(ChatFormatting.GRAY).forGoggles(tooltip);
        CCBLang.number(totalStress).translate("gui.goggles.unit.stress").style(ChatFormatting.AQUA).space().add(CCBLang.translate("gui.goggles.at_current_speed").style(ChatFormatting.DARK_GRAY).component()).forGoggles(tooltip, 1);
    }
}
