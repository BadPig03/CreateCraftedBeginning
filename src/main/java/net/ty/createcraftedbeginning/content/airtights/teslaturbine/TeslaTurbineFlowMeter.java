...
public class TeslaTurbineFlowMeter {
    ...
    public long fill(GasStack resource, GasAction action, boolean isClockwise) {
        if (resource.isEmpty()) {
            return 0;
        }
    ...
    public void reset(boolean resetGasType) {
        boolean hadRuntimeState = hasRuntimeState(resetGasType);
    ...
    private boolean hasRuntimeState(boolean resetGasType) {
        return netFlow != 0 || absoluteFlow != 0 || gatheredClockwise != 0 || gatheredCounterClockwise != 0 || currentSampleIndex != 0 || ticksUntilNextSample != TeslaTurbineUtils.FLOW_SAMPLE_RATE || hasMixedGases || resetGasType && !gasType.isEmpty();
    }
