...
public class HorizontalAirtightTankItem extends AirtightTankItem {
    ...
    protected int countTanksToPlace(Level level, BlockPos startPos, Axis tankAxis, int width) {
        int tanksToPlace = 0;
    ...
    protected void placeTankLayer(BlockPlaceContext context, Level level, BlockPos startPos, Direction face, Axis tankAxis, int width) {
        for (int uOffset = 0; uOffset < width; uOffset++) {
            for (int vOffset = 0; vOffset < width; vOffset++) {
                BlockPos targetPos = offsetLayer(startPos, tankAxis, uOffset, vOffset);
                BlockState targetState = level.getBlockState(targetPos);
    ...
    protected boolean isCompatibleHorizontalTank(BlockState candidateState, Axis tankAxis) {
