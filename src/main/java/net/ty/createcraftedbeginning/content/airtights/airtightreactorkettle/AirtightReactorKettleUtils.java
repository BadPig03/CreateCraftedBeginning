...
public final class AirtightReactorKettleUtils {
    ...
    static Optional<ReactorKettleRecipe> getMatchingRecipe(AirtightReactorKettleBlockEntity kettle) {
        if (kettle.isEmpty()) {
    ...
    static float getTotalFluidUnits(SmartFluidTankBehaviour inputTank, SmartFluidTankBehaviour outputTank, float partialTicks) {
        return getFluidUnits(inputTank, partialTicks) + getFluidUnits(outputTank, partialTicks);
    }

    static int getTotalFluidCapacity(SmartFluidTankBehaviour inputTank, SmartFluidTankBehaviour outputTank) {
    ...
    static boolean matchCraftingRecipe(AirtightReactorKettleBlockEntity kettle, CraftingRecipe recipe) {
        return canApplyCraftingRecipe(kettle, recipe);
    }

    static boolean applyCraftingRecipe(AirtightReactorKettleBlockEntity kettle, CraftingRecipe recipe) {
        Level level = kettle.getLevel();
    ...
    private static boolean canApplyCraftingRecipe(AirtightReactorKettleBlockEntity kettle, CraftingRecipe recipe, CraftingInput craftingInput, List<ItemStack> craftingOutputs) {
        Level level = kettle.getLevel();
        return level != null && recipe.matches(craftingInput, level) && !craftingOutputs.isEmpty() && kettle.testRecipeFilter(craftingOutputs.getFirst()) && kettle.acceptOutputs(craftingOutputs, new ArrayList<>(), new ArrayList<>());
    }
