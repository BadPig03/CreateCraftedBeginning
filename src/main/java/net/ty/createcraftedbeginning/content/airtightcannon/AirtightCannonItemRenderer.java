...
public class AirtightCannonItemRenderer extends CustomRenderedItemModelRenderer {
    public static final IItemDecorator DECORATOR = (guiGraphics, font, stack, xOffset, yOffset) -> {
        ...
        AirtightCannonItem.Ammo ammo = AirtightCannonItem.getAmmo(player, stack);
        if (ammo == null || CCBItems.AIRTIGHT_CANNON.is(ammo.stack())) {
            return false;
        }
    ...
    protected void render(ItemStack stack, CustomRenderedItemModel model, PartialItemModelRenderer renderer, ItemDisplayContext transformType, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        if (player != null) {
            ...
            if (inMainHand || inOffHand) {
                ...
                float speed = CreateCraftedBeginningClient.AIRTIGHT_CANNON_RENDER_HANDLER.getAnimation(inMainHand ^ leftHanded, AnimationTickHolder.getPartialTicks());
