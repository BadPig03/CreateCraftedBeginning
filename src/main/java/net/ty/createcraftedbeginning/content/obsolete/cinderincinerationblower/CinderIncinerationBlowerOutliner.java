...
public class CinderIncinerationBlowerOutliner {
    ...
    public void tick() {
        ...
        outliner.chaseAABB(outline, selectionBox).colored(0xDDC166).withFaceTextures(AllSpecialTextures.CHECKERED, AllSpecialTextures.HIGHLIGHT_CHECKERED).lineWidth(1 / 16.0f);
    }

    private @NotNull AABB calculateSelectionBox(BlockPos center, int edge) {
        double halfEdge = edge / 2.0f;
    ...
    private boolean isActive() {
        ...
        return player != null && isPresent() && AllItems.GOGGLES.isIn(player.getItemBySlot(EquipmentSlot.HEAD));
    }
