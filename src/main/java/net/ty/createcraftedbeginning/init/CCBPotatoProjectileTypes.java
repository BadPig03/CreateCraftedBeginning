...
public class CCBPotatoProjectileTypes {
    public static void bootstrap(BootstrapContext<PotatoCannonProjectileType> context) {
        ...
        register(context, "builders_tea_ice_cream", new Builder().damage(5).addItems(CCBItems.BUILDERS_TEA_ICE_CREAM.asItem()).knockback(0.25f).reloadTicks(20).renderTowardMotion(140, 2).velocity(1.5f).soundPitch(1.0f).onEntityHit(new PotionEffect(MobEffects.DIG_SPEED, 1, 300, false)).build());
        ...
        register(context, "golden_ice_cream", new Builder().damage(16).addItems(CCBItems.GOLDEN_ICE_CREAM.asItem()).knockback(0.25f).reloadTicks(20).renderTowardMotion(140, 2).velocity(1.5f).soundPitch(1.0f).build());
