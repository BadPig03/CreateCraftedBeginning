...
public class DissipationCategory extends CCBRecipeCategory<DissipationRecipe> {
    ...
    private static void addGasInputSlot(IRecipeLayoutBuilder builder, SizedGasIngredient gasIngredient) {
        ...
        builder.addSlot(RecipeIngredientRole.INPUT, 27, 51).setFluidRenderer(FluidType.BUCKET_VOLUME, false, 16, 16).setBackground(getRenderedSlot(), -1, -1).addIngredients(CCBJEI.GAS_STACK, fullStacks).addRichTooltipCallback((recipeSlotView, tooltip) -> gasStackList.stream().map(stack -> Component.translatable("jei.tooltip.gas.amount", stack.getAmount()).withStyle(ChatFormatting.GRAY)).forEach(tooltip::add));
    }

    private static void addGasOutputSlot(IRecipeLayoutBuilder builder, GasStack outputGas) {
        builder.addSlot(RecipeIngredientRole.OUTPUT, 131, 50).setFluidRenderer(FluidType.BUCKET_VOLUME, false, 16, 16).setBackground(getRenderedSlot(), -1, -1).addIngredient(CCBJEI.GAS_STACK, outputGas.copyWithAmount(FluidType.BUCKET_VOLUME)).addRichTooltipCallback((recipeSlotView, tooltip) -> tooltip.add(Component.translatable("jei.tooltip.gas.amount", outputGas.getAmount()).withStyle(ChatFormatting.GRAY)));
    }
}
