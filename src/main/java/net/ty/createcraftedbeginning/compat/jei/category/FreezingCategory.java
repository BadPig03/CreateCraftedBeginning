...
public class FreezingCategory extends CCBRecipeCategory<FreezingRecipe> {
    private static final int SCALE = 24;

    public FreezingCategory(@NotNull Info<FreezingRecipe> info) {
        super(info);
    }

    public static @NotNull Supplier<ItemStack> getCatalystStack() {
        ItemStack stack = AllBlocks.ENCASED_FAN.asStack();
        stack.set(DataComponents.CUSTOM_NAME, CCBLang.translateDirect("recipe.fan_freezing.fan").withStyle(style -> style.withItalic(false)));
        return () -> stack;
    }

    @Override
    protected void draw(@NotNull FreezingRecipe recipe, IRecipeSlotsView iRecipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
    ...
    @Override
    protected void setRecipe(@NotNull IRecipeLayoutBuilder builder, @NotNull FreezingRecipe recipe, IFocusGroup focuses) {
