...
public class AnimatedGasInjectionChamber extends AnimatedKinetics {
    ...
    private float getNozzleSqueeze(float cycle) {
        if (cycle < 20) {
            return Mth.sin((float) (cycle / 40f * Math.PI)) * 15;
        } else if (cycle > 60 && cycle < 80) {
            return Mth.sin((float) ((cycle - 40) / 40f * Math.PI)) * 15;
        } else if (cycle >= 80) {
            return 0;
        }
        return 15;
    ...
    private float getNozzleTopSqueeze(float cycle) {
        ...
        if (cycle < 30) {
            return Mth.sin((float) ((cycle - 20) / 60f * Math.PI)) * 7;
        }
        else if (cycle > 50) {
            return Mth.sin((float) (cycle / 60f * Math.PI)) * 7;
        }
        return 3.5f;
    }
}
