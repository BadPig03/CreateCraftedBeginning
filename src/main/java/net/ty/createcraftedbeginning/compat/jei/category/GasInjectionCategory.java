...
public class GasInjectionCategory extends CCBRecipeCategory<GasInjectionRecipe> {
    ...
    public void setRecipe(@NotNull IRecipeLayoutBuilder builder, @NotNull GasInjectionRecipe recipe, IFocusGroup focuses) {
        ...
        GasStack gasStack = recipe.getRequiredGas().getFirstGas();
        builder.addSlot(RecipeIngredientRole.INPUT, 27, 32).setBackground(getRenderedSlot(), -1, -1).addIngredient(JEIPlugin.GAS_STACK, gasStack.copyWithAmount(FluidType.BUCKET_VOLUME)).setFluidRenderer(FluidType.BUCKET_VOLUME, false, 16, 16).addRichTooltipCallback((recipeSlotView, tooltip) -> tooltip.add(Component.translatable("jei.tooltip.gas.amount", gasStack.getAmount()).withStyle(ChatFormatting.GRAY)));
