...
public interface AirtightArmorsHandler {
    ...
    default void appendHelmetHoverText(ItemStack helmet, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
    ...
    default void appendChestplateHoverText(ItemStack helmet, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
    ...
    default void appendLeggingsHoverText(ItemStack leggings, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        float consumptionMultiplier = getConsumptionMultiplier()[2];
        MutableComponent advancedConsumptionMultiplier = flag.isAdvanced() ? CCBLang.text(" [x" + getRenderStr(consumptionMultiplier) + ']').component() : Component.empty();
        tooltip.add(CCBLang.translate("gui.tooltips.gas_tools.gas_consumption", getRenderStr(consumptionMultiplier * 100)).add(advancedConsumptionMultiplier.withStyle(ChatFormatting.GRAY)).style(ChatFormatting.DARK_GREEN).component());
    }

    default void appendBootsHoverText(ItemStack boots, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
