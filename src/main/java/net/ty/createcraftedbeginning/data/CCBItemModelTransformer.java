    ...
    public static <T extends Item, P> @NotNull NonNullFunction<ItemBuilder<T, P>, ItemBuilder<T, P>> gasCanisterPack() {
        return builder -> builder.model((context, provider) -> {
            ...
            for (GasCanisterPackType type : GasCanisterPackType.values()) {
                ...
                itemBuilder.override().predicate(GasCanisterPackType.TYPE, index).model(provider.getBuilder(context.getName() + '_' + index).parent(new UncheckedModelFile("item/generated")).texture("layer0", CreateCraftedBeginning.asResource("item/gas_canister_pack" + type.getSerializedName()))).end();
            }
        });
    }
