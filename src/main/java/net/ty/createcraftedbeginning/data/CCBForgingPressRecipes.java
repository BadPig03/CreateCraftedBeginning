...
public class CCBForgingPressRecipes extends ForgingPressRecipeGen {
    ...
    GeneratedRecipe CRYING_OBSIDIAN_CHUNK = create("crying_obsidian_chunk", builder -> builder.require(Tags.Items.OBSIDIANS_CRYING).output(CCBItems.CRYING_OBSIDIAN_CHUNK));

    GeneratedRecipe PAPER = create("paper", b -> b.require(Tags.Items.CROPS_SUGAR_CANE).output(Items.PAPER));
    GeneratedRecipe DIRT_PATH_FROM_DIRT = create("dirt_path_from_dirt", builder -> builder.require(Ingredient.of(Items.DIRT, Items.COARSE_DIRT, Items.ROOTED_DIRT, Items.MYCELIUM, Items.PODZOL)).output(Items.DIRT_PATH));
    ...
    GeneratedRecipe BRASS_SHEET = create("brass_sheet", builder -> builder.require(ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", "ingots/brass"))).output(AllItems.BRASS_SHEET));
